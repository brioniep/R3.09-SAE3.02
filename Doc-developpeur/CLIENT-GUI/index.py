...
class MaFenetre(QWidget):
    ...
    def quitter_app(self):
        """
        Quitte l'application en se déconnectant du serveur si l'utilisateur est connecté.

        Variables :
            est_connecte (bool) : Indicateur de l'état de connexion de l'utilisateur au serveur.

        Méthodes appelées :
            deconnexion_du_serveur() : Déconnecte l'utilisateur du serveur si nécessaire.
            close() : Ferme la fenêtre de l'application.
        """
    ...
    def aide_util(self):
        """
        Affiche une fenêtre d'aide avec des instructions sur l'utilisation de l'application.

        Méthodes appelées :
            QMessageBox() : Crée une boîte de message d'information avec des instructions.
            setIcon() : Définit l'icône à afficher dans la boîte de message.
            setWindowTitle() : Définit le titre de la boîte de message.
            setText() : Définit le texte du message à afficher.
            setStandardButtons() : Définit les boutons standard de la boîte de message.
            exec() : Exécute et affiche la boîte de message.
        """
    ...
    def connexion_au_serveur(self):
        """
        Gère la connexion au serveur en utilisant l'IP et le port fournis.

        Variables :
            ip_input (QLineEdit) : Champ de saisie pour l'IP du serveur.
            port_input (QLineEdit) : Champ de saisie pour le port du serveur.
            est_connecte (bool) : Indicateur de l'état de connexion au serveur.
            socket_client (socket.socket) : Socket client pour la connexion au serveur.
            historique_logs (list) : Liste des logs affichés dans l'interface.
            connexion_thread (QThread) : Thread pour gérer la connexion au serveur.
            recepteur_thread (QThread) : Thread pour recevoir les données du serveur.

        Méthodes appelées :
            QMessageBox.warning() : Affiche un message d'avertissement si l'IP ou le port est incorrect.
            socket.socket() : Crée un socket pour se connecter au serveur.
            socket.connect() : Tente de se connecter au serveur avec l'IP et le port.
            QThread.sleep() : Fait une pause de 5 secondes entre les tentatives de connexion.
            setEnabled() : Active ou désactive certains boutons selon l'état de la connexion.
            run() : Démarre le thread de connexion et le thread de réception des données.
        """
    ...
    def deconnexion_du_serveur(self):
        """
        Déconnecte l'utilisateur du serveur et arrête le thread de réception des données.

        Variables :
            est_connecte (bool) : Indicateur de l'état de connexion de l'utilisateur au serveur.
            socket_client (socket.socket) : Le socket utilisé pour la connexion au serveur.
            historique_logs (list) : Liste des logs affichés dans l'interface.
            recepteur_thread (QThread) : Thread responsable de la réception des données depuis le serveur.

        Méthodes appelées :
            socket_client.close() : Ferme la connexion réseau avec le serveur.
            setEnabled() : Désactive les boutons pour la sélection et l'envoi de fichiers après la déconnexion.
            quit() : Arrête le thread de réception des données.
        """
        if self.est_connecte:
            try:
    ...
    def selectionner_fichier(self):
        """
        Ouvre une boîte de dialogue pour sélectionner un fichier et vérifie son format.

        Variables :
            chemin (QLineEdit) : Champ de texte où le chemin du fichier sélectionné est affiché.
        
        Méthodes appelées :
            QFileDialog.getOpenFileName() : Ouvre une boîte de dialogue pour sélectionner un fichier.
            os.path.splitext() : Sépare le nom du fichier de son extension.
            QMessageBox.warning() : Affiche un message d'avertissement si le format du fichier est incorrect.
            setText() : Affiche le chemin du fichier sélectionné dans le champ de texte.
        """
    ...
    def envoyer_fichier(self):
        """
        Envoie un fichier sélectionné au serveur si la connexion est active.

        Variables :
            est_connecte (bool) : Indicateur de l'état de connexion de l'utilisateur au serveur.
            chemin (QLineEdit) : Champ de texte affichant le chemin du fichier sélectionné.
            historique_logs (list) : Liste des logs affichés dans l'interface.
            socket_client (socket.socket) : Le socket utilisé pour la communication avec le serveur.

        Méthodes appelées :
            os.path.isfile() : Vérifie si le fichier spécifié existe.
            open() : Ouvre le fichier en mode binaire pour la lecture.
            socket_client.sendall() : Envoie des données au serveur via le socket.
            clear() : Efface le texte affiché dans le champ de texte pour le chemin du fichier.
    ...
    def recevoir_donnees(self):
        """
        Réceptionne les données envoyées par le serveur.

        Variables :
            est_connecte (bool) : Indicateur de l'état de la connexion avec le serveur.
            socket_client (socket.socket) : Le socket utilisé pour recevoir les données envoyées par le serveur.

        Méthodes appelées :
            socket_client.recv() : Permet de recevoir les données depuis le serveur.
            decode() : Décode les données reçues en texte (utf-8).
            split() : Divise la chaîne de caractères reçue en deux parties : le nom du fichier et son contenu.
            afficher_message() : Affiche le nom du fichier et son contenu dans l'interface.
    ...
    def afficher_message(self, nom_fichier, contenu_fichier):
        """
        Affiche un message avec le nom et le contenu du fichier reçu, en fonction de son extension.

        Variables :
            nom_fichier (str) : Le nom du fichier reçu, utilisé pour déterminer son extension et formater le message.
            contenu_fichier (str) : Le contenu du fichier reçu, qui sera affiché après le prompt coloré.
            extention (str) : L'extension du fichier, utilisée pour déterminer le format et la couleur du prompt.
            prompt (str) : Le message formaté avec la couleur du prompt, basé sur l'extension du fichier.

        Méthodes appelées :
            os.path.splitext() : Permet d'extraire l'extension du nom du fichier.
            append() : Ajoute le message formaté et le contenu du fichier à `fichiers_recus` et à `historique_logs`.
        """
